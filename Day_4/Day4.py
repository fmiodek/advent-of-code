...
with open(os.path.join(sys.path[0], "input.txt"), "r") as f:
    ...
    for call in calls:
        for c in cards:
            for i in range(5):
                ...
                # if a row of matrix or the transposed matrix is empty, we have a winning card
                if len(c.matrix[i]) == 0 or len(c.transposed[i]) == 0:
                    if len(solved) < 99 and c not in solved:
                        solved.append(c)
                    elif len(solved) == 99 and c not in solved:
                        last_card = c.matrix
                        last_call = call
                        end = True
                        break
    ...
    # sum up the ramining card values
    sum = 0
    for i in range(len(last_card)):
        for j in range(len(last_card[i])):
            sum += int(last_card[i][j])
